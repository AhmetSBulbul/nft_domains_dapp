...
class Domains extends _i1.GeneratedContract {
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<_i1.EthereumAddress> domains(
    String $param3, {
    _i1.BlockNum? atBlock,
  }) async {
    final function = self.abi.functions[3];
    assert(checkSignature(function, '26449235'));
    final params = [$param3];
    final response = await read(
      function,
      params,
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<_i1.EthereumAddress> getOwner(
    String domain, {
    _i1.BlockNum? atBlock,
  }) async {
    final function = self.abi.functions[5];
    assert(checkSignature(function, '4aaf4a12'));
    final params = [domain];
    final response = await read(
      function,
      params,
  ...
  Future<String> getRecord(
    ...
    assert(checkSignature(function, '11dd8845'));
    final params = [domain];
    final response = await read(
      function,
      params,
  ...
  Future<BigInt> price(
    ...
    assert(checkSignature(function, 'fe2c6198'));
    final params = [domain];
    final response = await read(
      function,
      params,
  ...
  Future<String> records(
    ...
    assert(checkSignature(function, '541e771d'));
  ...
  Future<String> register(
    ...
    assert(checkSignature(function, 'f2c298be'));
    final params = [domain];
    return write(
      credentials,
      transaction,
  ...
  Future<String> setRecord(
    ...
    assert(checkSignature(function, 'c1880a98'));
    final params = [
      domain,
      record,
    ];
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<String> tld({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[19];
    assert(checkSignature(function, '2d551432'));
    final params = [];
    final response = await read(
      function,
      params,
